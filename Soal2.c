...
static int xmp_read(const char *path, char *buf, size_t size, off_t offset,struct fuse_file_info *fi)
{
    ...
    if (fd1308 == -1)
        ...
        if(strcmp(ekstensii, ".pdf") == 0 || strcmp(ekstensii, ".doc") == 0 || strcmp(ekstensii, ".txt") == 0){
            ...
            char folderBaru1323[500];
            
            //ambil direktori
            sprintf(folderBaru1323,"%s/rahasia",dirpath);
            
             //direktori secret ?
            struct stat s;
            if (stat(folderBaru1323, &s) != 0)          //ga ada secret
                mkdir(folderBaru1323, 0777); //buat secret
                    
            sprintf(iniperintah, "mv %s %s/%s.ditandai && chmod 000 %s/%s.ditandai", filepath, folderBaru1323,path,folderBaru1323,path); //rename+move+chmod
            system(iniperintah);
            return -errno;                                   
        }
